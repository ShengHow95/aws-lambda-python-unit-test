...
class TestAdminCreateEvent():
    ...
    def test_checkSeoUrlExistence(self, dynamodb_resource):
        ...
        """ SeoUrl Not Exists """
        response = lambda_function.checkSeoUrlExistence('test')
        assert response == []

        """ SeoUrl Exists """
        response = lambda_function.checkSeoUrlExistence('test1')
        data = DynamoDB_Get_Item(dynamodb_resource, EVENT_TABLE, EVENT_TABLE_PK, 'test1')
        assert response != []
        assert response == [data]

        response = lambda_function.checkSeoUrlExistence('test2')
    ...
    def test_lambda_handler(self, lambda_context, mocker):
        ...
        """ All OK """
        mocker.patch('lambda.functions.AdminCreateEvent.lambda_function.checkSeoUrlExistence', return_value=[])
        ...
        mocker.patch('lambda.functions.AdminCreateEvent.lambda_function.checkSeoUrlExistence', return_value=InitialEventData)
        mocker.patch('lambda.functions.AdminCreateEvent.lambda_function.create_event', return_value=None)
        response = lambda_function.lambda_handler(SampleLambdaEvent1, lambda_context)
        assert response['statusCode'] == 400
